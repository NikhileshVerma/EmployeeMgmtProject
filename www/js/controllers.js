...
.controller('TasklistsCtrl' ,function($scope, $http, taskService) {
    
taskService.get_task().success(function(data) {
                $scope.Task = data;
                console.log(data);
    ...
    /* $scope.click = function(taskid) {
         $scope.taskid=taskid;
        // alert($scope.taskid);
         $state.go('app.camera', { 'task_id': $scope.taskid });
     };*/
...
.controller('CameraCtrl' ,function($scope, taskService, $http,  $localStorage, $sessionStorage, $stateParams, $state) {
    
    $scope.task_id = $stateParams.taskID;
    alert("hello="+$scope.task_id);
    ...
    $scope.uploadFile = function(text, drop_down) {
        ...
        options.fileName=$scope.task_id;
       ...
       // options.params = $scope.params;
        ...
        if(navigator.connection.type == Connection.NONE) {
              ...
              /* localStorage.setItem("Id", $scope.task_id); 
            ...
            var taskid=$scope.task_id;
            var image=$scope.myImg;
            var data=$scope.data;
            var newMsg={
                id: taskid,
                image: image,
            ...
            storeArr = JSON.parse(localStorage.getItem('session'));
            alert("storeArr="+storeArr);
            
           var data= JSON.stringify(newMsg);
            alert("data"+data);
            storeArr.push({taskid, image, data });
            taskid="";
            myImg="";
            data="";
           alert("store=" +storeArr);
           localStorage.setItem("session", JSON.stringify(storeArr));
           alert("dataa" + localStorage.getItem("session"));
...
alert(JSON.stringify(response));
        ...
        $scope.allAssets = response.assets;
      //   $scope.data.x.inputTag = "";
...
if(window.Connection) {
  alert("out of if");
            if(navigator.connection.type == Connection.NONE) {
               ...
               // alert("locally storeed data is"+data);
              }
else{
  alert("in post");
    ...
    $scope.submitForm = function() {       
        ...
        if(window.Connection) {
            if(navigator.connection.type == Connection.NONE) {
                ...
                alert("The internet is disconnected on your device.");  
                window.localStorage.setItem("data", $scope.data);
                window.localStorage.setItem("image", $scope.myImg);
                alert("data="+localStorage.getItem("data"));
                alert("image=" + localStorage.getItem("image"));
}
